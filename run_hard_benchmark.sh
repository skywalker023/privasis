#!/bin/bash
#
# Test & evaluate sanitization model on Privasis-Zero hard test set
#
# Two-phase pipeline:
...
#
# Downloads nvidia/Privasis-Zero hard/test from HuggingFace on first run.
#
# Usage:
#   ./run_hard_benchmark.sh
#
#   # Override defaults:
#   MODEL=openai/gpt-oss-120b ./run_hard_benchmark.sh
#
...
HF_SUBSET="${HF_SUBSET:-hard}"
HF_SPLIT="${HF_SPLIT:-test}"
SANITIZED_OUTPUT="${SANITIZED_OUTPUT:-${MODEL##*/}_hard_test.jsonl}"
EVALUATION_OUTPUT="${EVALUATION_OUTPUT:-${MODEL##*/}_hard_test_eval_outputs.jsonl}"
